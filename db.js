...
async function insertClient(client) {
    ...
    return await conn.query(sql, [client.name,
        client.age,
        client.state
    ]);
}
...
async function deleteClient(idclient) {
    const conn = await connect();
    return await conn.query('DELETE FROM crud.clients WHERE idclient=?;', [idclient]);
}

module.exports = {selectClients, insertClient, updateClient, selectClient, deleteClient}